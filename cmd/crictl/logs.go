...
var logsCommand = &cli.Command{
	...
	Action: func(ctx *cli.Context) (retErr error) {
		...
		logPath := status.Status.GetLogPath()
		if logPath == "" {
			return fmt.Errorf("The container has not set log path")
		}
		if previous {
			containerAttempt := status.Status.GetMetadata().Attempt
			if containerAttempt == uint32(0) {
				return fmt.Errorf("Previous terminated container %s not found", status.Status.GetMetadata().Name)
			}
		...
		return logs.ReadLogs(readLogCtx, logPath, status.Status.GetId(), logOptions, runtimeService, os.Stdout, os.Stderr)
	},
}
